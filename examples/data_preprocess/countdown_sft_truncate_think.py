...
def truncate_think(think_text, tokenizer, max_think_tokens=128):
    """
    将 <think>...</think> 内的内容截断到 max_think_tokens。
    保留结构：<think> 截断后的内容 </think>
    """
    think_text = think_text.strip()

    # tokenize think 部分
    think_ids = tokenizer.encode(think_text, add_special_tokens=False)
    # print(f"len(think_ids): {len(think_ids)}")
    # ipdb.set_trace()
    # 如果长于阈值则截断
    if len(think_ids) > max_think_tokens:
        think_ids = think_ids[:max_think_tokens]

    # decode 回文本
    new_think_text = tokenizer.decode(think_ids, skip_special_tokens=True)
    return new_think_text


def clean_answer_truncate_think(text, tokenizer, max_think_tokens=128):
    # 找到 <think> ... </think>
    m = re.search(r"<think>(.*?)</think>", text, re.S)
    if not m:
        return text  # 无 think 就直接返回
    
    think_raw = m.group(1)
    # ipdb.set_trace()
    think_trunc = truncate_think(think_raw, tokenizer, max_think_tokens=128)
    # ipdb.set_trace()
    # 替换回文本
    new_text = text.replace(
        f"<think>{think_raw}</think>",
        f"<think>{think_trunc}</think>"
    )
    # ipdb.set_trace()
    return new_text
...
if __name__ == "__main__":
    ...
    parser.add_argument("--local_dir", default="/users/gwang16/Jillian/TinyZero/data/countdown_sft_truncate_think")
    parser.add_argument("--max_think_tokens", type=int, default=128)
    ...
    # Load tokenizer
    # -------------------------
    tokenizer_path = args.model_path if args.model_path is not None else args.model_name
    ...
    def extract_map(example):
        ...
        if role == "assistant":
            ...
            # ipdb.set_trace()
            # truncate think
            answer = clean_answer_truncate_think(answer, tokenizer, max_think_tokens=args.max_think_tokens)
            # ipdb.set_trace()
    ...
    # Save
    # -------------------------
    local_dir = os.path.expanduser(args.local_dir + '_maxthinktokens' + str(args.max_think_tokens))
